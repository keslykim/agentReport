...
function App() {
    ...
    const parseOcrText = (text) => {
        ...
        lines.forEach((line, idx) => {
            ...
            if (numbers) {
                numbers.forEach(num_str => {
                    const digits_only = num_str.replace(/[^\d]/g, '');
                    if (digits_only.startsWith('0')) return;
                    const clean_num = parseInt(digits_only);
        ...
        // Priority 2: Common store name suffixes (first 8 lines)
        if (!summary) {
            const store_keywords = ["농협", "하나로마트", "축협", "수협", "신협", "이마트24", "마트", "식당", "푸드", "커피", "카페", "제과", "헤어", "의원", "약국", "편의점", "코리아", "상사", "유통", "갈비", "한우", "피자", "치킨", "스토어", "올리브영", "다이소", "이마트", "홈플러스", "롯데마트", "CU", "GS25", "세븐일레븐"];
            for (let line of lines.slice(0, 8)) {
                if (store_keywords.some(k => line.toUpperCase().includes(k.toUpperCase()))) {
                    if (!["전화", "대표", "등록번호", "일자", "주소"].some(k => line.includes(k))) {
        ...
        // Priority 3: first clean non-number line (first 6 lines)
        if (!summary) {
        ...
        if (summary) {
            if ((summary.toLowerCase().includes("fad") || summary.toLowerCase() === "fad") && (text.toLowerCase().includes("nonghyup") || text.includes("농협"))) {
                summary = summary.replace(/FAD/i, "농협");
            }
        }
        if (!summary && (text.toLowerCase().includes("nonghyup") || text.includes("농협"))) {
            summary = "농협";
        }
    ...
    */}
    const expectedTax = hasTransactions ? (dashboard.total_estimated_tax || dashboard.expected_tax) : 0;
                ...
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl relative animate-in fade-in zoom-in-95 duration-200">
                        ...
                        */}
                        ...
                        <div className="space-y-4 border-y border-outline-variant py-4 text-base">
                            <div className="flex justify-between font-medium">
                                <span className="text-outline">예상 부가가치세</span>
                                <span>{dashboard.recent_count === 0 ? "0" : (dashboard.estimated_vat || 0).toLocaleString()} 원</span>
                            </div>
                            <div className="flex justify-between font-medium">
                                <span className="text-outline">예상 종합소득세 과세표준</span>
                                <span>{dashboard.recent_count === 0 ? "0" : (dashboard.taxable_income || 0).toLocaleString()} 원</span>
                            </div>
                            <div className="flex justify-between font-bold text-secondary">
                                <span className="flex items-center gap-1">
                                    <span className="material-symbols-outlined text-sm font-bold">check_circle</span>
                                ...
                                <span>- {dashboard.recent_count === 0 ? "0" : (dashboard.valid_expenses_total || 0).toLocaleString()} 원</span>
                            </div>
                            <div className="border-t border-outline-variant pt-4 flex justify-between text-xl font-black text-primary">
                                <span>최종 예상 납부 세액</span>
                                <span>{expectedTax.toLocaleString()} 원</span>
                            </div>
                        </div>
                        */}
                        <div className="space-y-4 border-y border-outline-variant py-4 text-base">
                            {/* 1. 매출 및 경비 */}
                            <div className="flex justify-between font-medium">
                                <span className="text-outline">총 예상 매출액</span>
                                <span>{dashboard.recent_count === 0 ? "0" : (dashboard.total_revenue || 0).toLocaleString()} 원</span>
                            </div>
                            <div className="flex justify-between font-bold text-secondary">
                                <span className="flex items-center gap-1">
                                    <span className="material-symbols-outlined text-sm font-bold">check_circle</span>
                            ...
                            {/* 2. 과세표준 산출 */}
                            <div className="flex justify-between font-medium border-t border-outline-variant/50 pt-3">
                                <span className="text-outline">예상 종합소득세 과세표준</span>
                                <span>{dashboard.recent_count === 0 ? "0" : (dashboard.taxable_income || 0).toLocaleString()} 원</span>
                            </div>
                            ...
                            <div className="flex justify-between font-medium">
                                <span className="text-outline">예상 종합소득세</span>
                                <span>{dashboard.recent_count === 0 ? "0" : (dashboard.income_tax || 0).toLocaleString()} 원</span>
                            </div>
                ...
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl text-center relative animate-in fade-in zoom-in-95 duration-200">
                        ...
                        <p className="text-lg text-on-surface-variant font-medium mb-4">
                            사장님의 숨은 절세 항목 <span className="text-secondary font-bold">{transactions.length}건</span>을 성공적으로 분석하여 장부에 반영했습니다.
                        </p>
