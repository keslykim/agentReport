...
function App() {
    ...
    const handleSendChatMessage = async (e) => {
        ...
        try {
            ...
            setChatMessages(prev => [...prev, { sender: 'seboki', text: '죄송합니다 사장님, 일시적인 오류가 발생했어요. 다시 한번 말씀해주시겠어요? 😢' }]);
        } finally {
                        ...
                        <div className="flex-grow overflow-y-auto p-4 space-y-4 bg-surface-container-low">
                            {chatMessages.map((msg, index) => (
                                <div key={index} className={`flex gap-2.5 ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                                    ...
                                    <div className={`p-3 text-base max-w-[75%] leading-relaxed whitespace-pre-line ${
                                        msg.sender === 'user' 
