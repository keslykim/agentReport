    ...
    try:
        ...
        for line in lines:
            ...
            if re.search(r'\d{3}-\d{2}-\d{5}', line) or "등록번호" in line:
                continue
            if re.search(r'\d{2,4}-\d{3,4}-\d{4}', line) or "전화" in line or "Tel" in line:
                continue
            if re.search(r'\d{4}[-/.]\d{2}[-/.]\d{2}', line) or "대표자" in line:
                continue

            # 점(.)과 쉼표(,)를 모두 고려하여 숫자 패턴 매칭
            numbers = re.findall(r'\d[\d,.\s]*\d|\d', line)
            for num_str in numbers:
                clean_num = int(re.sub(r'[^\d]', '', num_str))
                if 1000 <= clean_num <= 10000000:
                    weight = 0
                    if any(k in line for k in ["합계", "결제", "금액", "합 계", "총금액", "원"]):
                        weight += 10
                    if any(k in line for k in ["부가세", "과세", "면세", "할인", "거스름"]):
                        weight -= 5
        ...
        # 우선 순위 1: 명시적인 가맹점명/매장명/상호명 패턴 찾기 (첫 10라인 내에서 탐색)
        for line in lines[:10]:
            m = re.search(r'(?:가맹점명|가맹점|매장명|상호명|상호|판매점)\s*[:\]호]\s*([^\s].*)', line)
            if m:
                candidate = m.group(1).strip()
                # 불필요한 괄호쌍 제거 (예: (주)코리아세븐 -> 코리아세븐)
                candidate = re.sub(r'[\(\[\{].*?[\)\]\}]', '', candidate).strip()
                if candidate and candidate not in ["영수증", "지출", "신용카드", "승인민표", "매출전표", "고객용"]:
                    summary = candidate
                    break
        
        # 우선 순위 2: 명시적 패턴이 없는 경우, 첫 5라인 중 필터링을 통과한 첫 번째 라인을 상호명으로 간주
        if not summary:
            store_candidates = []
            for line in lines[:5]:
                if re.search(r'\d{3}-\d{2}-\d{5}', line) or "등록번호" in line:
                ...
                if re.search(r'\d{2,4}-\d{3,4}-\d{4}', line) or "전화" in line or "Tel" in line:
                    continue
                if re.search(r'\d{4}[-/.]\d{2}[-/.]\d{2}', line) or "주소" in line or "대표자" in line:
                    continue
                if re.match(r'^[\d\s\-,./:]+$', line):
                    continue
                if any(k in line for k in ["단가", "수량", "금액", "품명", "메뉴", "합계"]):
                    continue
                if any(k in line.lower() for k in ["http", "www", ".com", ".co.kr"]):
                    continue
                if any(k in line for k in ["영수증", "신용카드", "승인민표", "매출전표", "고객용", "1등", "감사합니다", "환영합니다", "어서오세요"]):
                    continue
                store_candidates.append(line)
